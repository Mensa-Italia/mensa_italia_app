#!/usr/bin/env bash
# Push iOS Swift + Android Kotlin tr() keys and fallbacks to Tolgee.
...
echo "▶ Extracting i18n keys from Swift + Kotlin source…"
python3 - <<'PY'
...
# Qui c'erano anche gli estrattori della web app (t("key","fallback") nei
# .ts/.tsx e data-i18n nei .astro). Il modulo webApp/ non esiste piu': le
# chiavi `web.*` restano su Tolgee, il push in KEEP non le tocca.
