...
class RangeDateTimeZone {
  ...
  setStart(DateTime start) {
    this.start = tz.TZDateTime(tz.local, start.year, start.month, start.day, start.hour, start.minute, start.second, start.millisecond, start.microsecond);
  }

  setEnd(DateTime end) {
    this.end = tz.TZDateTime(tz.local, end.year, end.month, end.day, end.hour, end.minute, end.second, end.millisecond, end.microsecond);
  }
  ...
  DateTime getStart() {
    return DateTime(start!.year, start!.month, start!.day, start!.hour, start!.minute, start!.second, start!.millisecond, start!.microsecond);
  }

  DateTime getEnd() {
    return DateTime(end!.year, end!.month, end!.day, end!.hour, end!.minute, end!.second, end!.millisecond, end!.microsecond);
  }
